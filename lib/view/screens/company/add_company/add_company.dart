              ...
              TextField(
                onChanged: (name) {
                  BlocProvider.of<CompanyBloc>(context)
                      .add(UpdateCompanyName(name: name));
                },
                decoration: InputDecoration(
                    labelText: 'Company name',
                    errorText: snapshots.snapshot1.hasError
                        ? snapshots.snapshot1.error.toString()
                        : null),
              ),
              TextField(
                onChanged: (description) {
                  BlocProvider.of<CompanyBloc>(context)
                      .add(UpdateCompanyDescription(description: description));
                },
                decoration: InputDecoration(
                    labelText: 'Company description',
                    errorText: snapshots.snapshot2.hasError
                        ? snapshots.snapshot2.error.toString()
                        : null),
              ),
              StreamBuilder(
                  stream: BlocProvider.of<CompanyBloc>(context).validateStreams,
                        ...
                        onPressed: () {
                          if (snapshot.data.runtimeType == bool &&
                            ...
                            BlocProvider.of<CompanyBloc>(context).add(
                                ErrorCompany(error: 'Form is not completed'));
                          }
                        },
